  ...
  <div class="hello">
    <div :class="[modalActive ? 'modal is-active' : 'modal']">
      ...
      <div class="modal-card">
        ...
        <footer class="modal-card-foot">
          <button @click="loadSavedQuotes" class="button is-success">Load em up</button>
          <button @click="loadFreshies" class="button">Start with freshies</button>
        </footer>
      </div>
    </div>
    ...
    <section class="section">
            ...
            <div class="columns">
              <div class="column">
                <div class="field">
                  ...
                  <div :class="[quoteLoading ? 'control is-loading' : 'control']">
                    <textarea
                      v-model="newText"
      ...
      <div class="container">
        <draggable
          v-model="items"
          :options="{ group: 'projects', delay: 250 }"
          @start="drag = true;"
          @end="drag = false;"
          class="group-1"
        >
          <div v-for="(item, index) in items" v-bind:key="item.id" class="the-item">
              ...
              <div class="message-body">
                <div class="columns">
                  ...
                  <div class="column">
                    ...
                    >{{ item.text }}</div>
                    <form v-if="editing == index + 1" @submit.prevent="commitEdit(index);">
                      <div class="columns">
                        <div class="column">
                        ...
                        <div class="column is-narrow">
                          <button @click="cancelEdit" class="button is-small is-warning">cancel</button>
                        </div>
                      </div>
                    </form>
                  </div>
                  <div class="column is-1">
                    <button @click="editTodo(index);" class="button is-small is-white edit-button">
                ...
                <div class="columns">
                  <div class="column">
                    <span class="tag is-light">Date Added: {{ item.date }}</span>
                  </div>
      ...
      <div class="container">
        <div class="column is-4 is-offset-8">
          ...
          >Delete all your quotes and get 3 new quotes.</button>
        </div>
      </div>
    </section>
  </div>
        ...
        this.$http
          ...
          .then(
            response => {
              // get body data
              let quote = {
                text: response.body[0].content,
                author: response.body[0].title,
                cat: response.body[0].link,
      ...
      if (this.addingTodo === true) {
        this.$http
          ...
          .then(
            response => {
              // get body data
              this.newText = response.body[0].content;
              this.newAuthor = response.body[0].title;
              this.newCat = response.body[0].link;
